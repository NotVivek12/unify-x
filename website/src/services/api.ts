...
// Safe fetch wrapper with timeout and generic error handling
...
// Helper to map backend shipment to frontend Shipment format
...
export const api = {
  getSystemStatus: async (isDemoMode: boolean): Promise<SystemStatus | null> => {
    ...
    // Safely map backend system status
  ...
  getShipmentLive: async (id: string, isDemoMode: boolean): Promise<SensorData | null> => {
    if (isDemoMode) {
      // Return mocked slight variations
      const shipment = mockShipments.find(s => s.shipment_id === id);
    ...
    // Extract nested object if wrapped
    const rawSensors = data.sensors || data.live_data || data;
    
    // Safely map to frontend SensorData type with fallbacks to avoid 'toFixed' undefined errors
    return {
